...
<body>
        ...
        $hit_query = "update QnA set hit = hit+1 where no = '$no' and id = '$id' "; //조회수 증가
    ...
    <div class="container" style="padding-top: 10px">
      <nav class="navbar navbar-default">
        ...
        if($member['user_id']==$data['user_id']){
            ...
            <button type="button" class="btn btn-danger" onclick="location.href='./qna_delete.php?no=<?=$data['no']?>&id=<?=$data['id']?>'">
            글삭제
            </button>
            <?
        }
        ...
        <div>
            <?
            // 댓글 자체를 가져온다.
            $comment_query = "select * from qna_comment where bbs1_no = '$no' order by no asc ";
        ...
        <?if($member['user_id']){?>
        <div>
            <form name="comment" action="qna_comment_post.php?no=<?=$no?>" method="POST">
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                ...
                <div class="modal-body">
                    <form name="edit" action="qna_edit_post.php" method="POST" enctype="multipart/form-data">
